...
<div class="wrapper">
    @foreach ($records as $record)
        ...
        <table>
            ...
            <tbody>
            @foreach ($record->salesItems as $index => $line)
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ \Illuminate\Support\Str::limit($line->item->name, 10) }}</td>
                    <td>{{ $line->qty }}</td>
                    <td>{{ 'EGP' }} {{ number_format($line->price, 2) }}</td>
                    <td>{{ 'EGP' }} {{ number_format($line->qty * $line->price, 2) }}</td>
                </tr>
            @endforeach
            </tbody>
        </table>
        ...
        <div class="summary">
            ...
            Total: {{ 'EGP' }} {{ number_format($record->total, 2) }}<br>
            Paid Amount: {{ 'EGP' }} {{ number_format($record->paid_amount, 2) }}<br>
            Money Back: {{ 'EGP' }} {{ number_format($record->paid_amount - $record->total, 2) }}<br>
        </div>
